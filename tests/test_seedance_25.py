...
def test_duration_and_reference_socket_caps():
    ...
    assert "reference_image31" not in optional


def test_model_max_duration():
    assert seedance._model_max_duration("seedance-2.0") == 15
    assert seedance._model_max_duration("seedance-2.0-fast") == 15
...
def test_validate_duration_for_model():
    ...
    seedance._validate_duration_for_model("seedance-2.5", "auto")
...
def test_validate_references_for_model_20_vs_25():
    ...
    with pytest.raises(ValueError, match="at most 12 reference files"):
        seedance._validate_references_for_model(
            "seedance-2.0",
            [object()] * 9,
            [object()] * 3,
            [object()] * 1,
...
def test_models_with_bitrate():
    ...
    assert "seedance-2.0-fast" in seedance.MODELS_WITH_BITRATE
