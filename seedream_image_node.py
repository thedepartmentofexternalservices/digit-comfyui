"""DIGIT Seedream Image node — ByteDance Seedream 5.0 image generation via fal.ai.

Auto-detects mode based on connected inputs:
- No image inputs connected → text-to-image
- Any image1..image10 connected → edit

Pro and lite variants are selected via the model dropdown. Batch generation
submits all jobs to the fal queue up front and polls, matching the DIGIT GPT
Image and DIGIT Seedance Video nodes. Uses FAL_KEY environment variable for
authentication.
"""
...
from .seedream_image_models import (
    ...
    MODELS_WITH_MAX_IMAGES,
    MODELS_WITH_OUTPUT_FORMAT,
    OUTPUT_FORMATS,
    SEEDREAM_IMAGE_APPS,
...
logger = logging.getLogger("DigitSeedreamImage")
...
class DigitSeedreamImage:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "model": (SEEDREAM_IMAGE_MODELS, {"default": DEFAULT_SEEDREAM_IMAGE_MODEL}),
                "image_size": (IMAGE_SIZES, {
                    "default": "auto_2K",
                    "tooltip": "auto_1K is pro only; auto_3K/auto_4K are lite only. custom uses custom_width/custom_height.",
                }),
                "output_format": (OUTPUT_FORMATS, {
                    "default": "jpeg",
                    "tooltip": "Pro only — lite endpoints choose their own format.",
                }),
                "num_images": ("INT", {
                    "default": 1,
                    "min": 1,
                    "max": MAX_NUM_IMAGES,
                    "tooltip": "Separate generations per API request. Total output = num_images x batch_count.",
                }),
                ...
                "seed": ("INT", {
                    ...
                    "min": 0,
                    "max": MAX_SEED,
                    "tooltip": "Re-run control only — the Seedream 5 API has no seed input. 0 regenerates on every queue.",
                }),
            },
            "optional": {
                **image_sockets,
                "max_images": ("INT", {
                    ...
                    "min": 1,
                    "max": MAX_NUM_IMAGES,
                    "tooltip": "Lite only: the model may return up to this many images per generation.",
                }),
                "custom_width": ("INT", {
                    "default": 2048,
                    "min": CUSTOM_DIM_MIN,
                    ...
                    "tooltip": "Used only when image_size is custom. Pro: total pixels 1024^2-2048^2. Lite: up to 4096x4096.",
                }),
                "custom_height": ("INT", {
                    "default": 2048,
                    "min": CUSTOM_DIM_MIN,
    ...
    def generate(self, prompt, model, image_size, output_format, num_images,
                 enable_safety_checker, seed, max_images=1, custom_width=2048,
                 custom_height=2048, batch_count=1, **kwargs):
        try:
            import fal_client
        except ImportError:
            raise ImportError(
                "fal-client is required for DigitSeedreamImage. "
        ...
        if model in MODELS_WITH_OUTPUT_FORMAT and image_size in SIZES_LITE_ONLY:
            raise ValueError(
                f"image_size '{image_size}' is lite only. "
                "Switch to seedream-5.0-lite or pick auto_1K/auto_2K."
            )
        if model not in MODELS_WITH_OUTPUT_FORMAT and image_size in SIZES_PRO_ONLY:
            raise ValueError(
                f"image_size '{image_size}' is pro only. "
                "Switch to seedream-5.0-pro or pick auto_2K and up."
            )
        ...
        app_id = SEEDREAM_IMAGE_APPS[model][mode]
        logger.info(f"[DigitSeedream] Mode: {mode} | App: {app_id}")
        ...
        if model in MODELS_WITH_OUTPUT_FORMAT:
            args["output_format"] = output_format
        if model in MODELS_WITH_MAX_IMAGES:
            args["max_images"] = int(max_images)
        ...
        if not any(job.get("image_count") for job in jobs):
            ...
            raise RuntimeError(f"All Seedream batch generations failed. {details}")
            ...
            while pending:
                ...
                for index in list(pending):
                    ...
                    try:
                        ...
                        logger.warning(
                            "[DigitSeedream] Status check failed for job %d: %s",
                            index + 1,
                            error,
                        )
                        continue
                    ...
                    try:
                        ...
                        if self._should_retry(error) and job["attempt"] <= MAX_AUTOMATIC_RETRIES:
                            ...
                            logger.warning(
                                "[DigitSeedream] Job %d failed on attempt %d; retrying in %ds: %s",
                                index + 1,
                                job["attempt"],
                                delay,
                                error,
        ...
        while job["attempt"] <= MAX_AUTOMATIC_RETRIES:
            try:
                ...
                if (
                    ...
                    logger.error(
                        "[DigitSeedream] Job %d submission failed after %d attempt(s): %s",
                        job["index"] + 1,
                        job["attempt"],
                        error,
                    )
                ...
                delay = 2 ** (job["attempt"] - 1)
                logger.warning(
                    "[DigitSeedream] Job %d submission failed on attempt %d; "
                    "retrying in %ds: %s",
                    job["index"] + 1,
                    job["attempt"],
                    delay,
    ...
    def _submit_job(fal_client, app_id, shared_args, job):
        job["attempt"] += 1
        logger.info(
            "[DigitSeedream] Submitting job %d, attempt %d to %s...",
            job["index"] + 1,
            job["attempt"],
            app_id,
        )
    ...
    def _cancel_jobs(jobs, pending):
        for index in pending:
            ...
            try:
                handle.cancel()
                logger.info("[DigitSeedream] Cancelled request %s", handle.request_id)
            except Exception as error:
                logger.warning(
                    "[DigitSeedream] Could not cancel request %s: %s",
    ...
    def _format_batch_status(self, mode, model, args, jobs):
        ...
        if "output_format" in args:
            lines.insert(3, f"Format: {args['output_format']}")
        if "max_images" in args:
            lines.insert(3, f"Max images per generation: {args['max_images']}")
        for job in jobs:
            result_seed = (
                job["result"].get("seed")
            ...
            if result_seed is not None:
                summary.append(f"seed={result_seed}")
